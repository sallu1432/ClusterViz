      ...
      <CardContent className="p-4 pt-8">
        <TooltipProvider>
          <div className="w-full h-96 relative">
            {/* Y-axis labels */}
            <div className="absolute left-0 top-0 h-full flex flex-col" style={{ width: '6rem' }}>
              {features.map((feature, i) => (
                <div
                  key={`y-${feature}`}
                  className="flex-1 flex items-center justify-end text-xs text-muted-foreground pr-2 text-right"
                  title={feature}
                >
            ...
            {/* X-axis labels */}
            <div className="absolute bottom-0 h-24 flex" style={{ left: '6rem', right: 0 }}>
              {features.map((feature, i) => (
                <div
                  key={`x-${feature}`}
                  className="flex-1 flex justify-center items-start pt-2 text-xs text-muted-foreground"
